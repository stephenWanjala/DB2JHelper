...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            config.setMaximumPoolSize(
                    Integer.parseInt(getEnv("DB2_MAX_POOL_SIZE", "50")));
            config.setMinimumIdle(Integer.parseInt(getEnv("DB2_MIN_IDLE", "10")));
            config.setConnectionTimeout(
                    Integer.parseInt(getEnv("DB2_CONNECTION_TIMEOUT", "30000")));
            config.setIdleTimeout(
                    Integer.parseInt(getEnv("DB2_IDLE_TIMEOUT", "6000000")));
            ...
            config.setLeakDetectionThreshold(
                    Integer.parseInt(getEnv("DB2_LEAK_DETECTION_THRESHOLD", "20000")));
        ...
        List<Customer> customers1 = client.select("LEDGER_NAME", "LEDGER_NUMBER").from("CUSTOMERS")
                .list()
                .stream()
                .map(rs -> new Customer(
                                (String) rs.get("LEDGER_NAME"),
                                (String) rs.get("LEDGER_NUMBER")
                        )
                ).toList();
        ...
        List<Customer> customers2 = client.query(sql).stream()
                .map(row -> new Customer((String) row.get("LEDGER_NAME"), (String) row.get("LEDGER_NUMBER"))).toList();
       ...
       Customer customer = new Customer((String) row.get("LEDGER_NAME"), (String) row.get("LEDGER_NUMBER"));
    ...
    record Customer(String ledgerName, String ledgerNumber) {}
}
